...
const g = svg.append('g').attr('font-size', fontSize);
const svgSize = {
...
function draggedNodeRenew(draggedNode, targetX, targetY, dura) {
  ...
  d3.select(draggedNode).each((d) => {
    d3.select(`path#path_${d.data.id}`).transition(t).attr('d', `${link({
      source: [-targetY + (d.parent ? d.parent.data.spanWidth : 0), -targetX],
      target: [0, 0],
    })}L${d.data.spanWidth},0`);
  });
}
...
function appendNode(enter) {
  ...
  const gNodeFroeign = gNode.append('foreignObject')
    .attr('class', d => `depth_${d.depth}`)
    .attr('y', -19)
    .attr('x', 0)
    .attr('width', 120)// 需要考虑
    .attr('height', 17);
  gNodeFroeign.append('xhtml:span')
  ...
  gNode.append('rect')
    ...
    .attr('y', -17 - 4)
    .attr('x', -4)
    .attr('width', d => d.data.spanWidth + 8)
    .attr('height', 16 + 8)
    .attr('rx', 3)
    .attr('ry', 3)
    .attr('stroke', 'blue')
  ...
  gNode.append('path')
    ...
    .attr('d', d => `${link({
      source: [
        (d.parent ? d.parent.y + d.parent.data.spanWidth : 0) - d.y,
...
function tree(d) {
  ...
  r.nodeHeight = 25;
  r.nodeWidth = svgSize.width / (r.height + 1);// r.height与叶子节点的最大距离
  // nodeSize设置了节点的大小（高宽)
